...
## GitHub 專案隱私規範 (GitHub Repository Privacy Policy)

**鐵律：所有新建立的 GitHub Repository 一律預設為 PRIVATE。**

1. **預設私有**：使用 gh repo create 或任何 GitHub CLI 指令建立新 Repository 時，必須加上 --private 參數，絕對禁止使用 --public。
2. **嚴禁私自公開**：不論任何理由，AI **絕對禁止**在未經使用者明確同意的情況下，將任何 Repository 的 Visibility 設定為 Public。
3. **有公開需求時的強制流程**：
   - 若任務需要將 Repo 設為 Public，必須**先以繁體中文向使用者說明**：為什麼需要公開、哪個 Repo、以及公開後的影響（任何人可見、可 Fork）。
   - 必須取得使用者**明確的文字同意**後，才可執行公開操作。
