...
def fetch_t86_data(date_str):
    ...
    try:
        ...
        if 'data' in data and data['data']:
            ...
            if buy_d_idx == -1: buy_d_idx = next(i for i, f in enumerate(fields) if '自營商買進' in f)
            sell_d_idx = next((i for i, f in enumerate(fields) if '自營商賣出股數' == f or '自營商賣出' in f and '自行' not in f and '避險' not in f), -1)
            if sell_d_idx == -1: sell_d_idx = next(i for i, f in enumerate(fields) if '自營商賣出' in f)
            ...
            print(f"  TWSE (上市) count: {len(data['data'])}")
    ...
    try:
        ...
        if 'aaData' in data and data['aaData']:
            for row in data['aaData']:
                ...
                d_buy = parse_int(row[11]); d_sell = parse_int(row[12])
            ...
            print(f"  TPEx (上櫃) count: {len(data['aaData'])}")
    except Exception as e:
...
def main():
    ...
    for filepath in files:
        ...
        if '0000' in all_stocks_data:
            print(f"{filename} already has T86 data (0000 exists). Skipping.")
            continue
